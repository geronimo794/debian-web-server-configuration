...
then
	...
	echo '# Required for Jenkins websocket agents
...
}' | sudo tee /etc/nginx/sites-available/$PORT_JENKINS.$URL_JENKINS > /dev/null
...
if [ ! -z "$URL_PROMOTHEUS" ]
then
	echo '# Generated by Rozikin Script
server {
...
}' | sudo tee /etc/nginx/sites-available/$PORT_PROMOTHEUS.$URL_PROMOTHEUS > /dev/null
...
}' | sudo tee /etc/nginx/sites-available/$PORT_GRAFANA.$URL_GRAFANA > /dev/null
